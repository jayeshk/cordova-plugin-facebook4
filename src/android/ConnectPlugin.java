...
private static final int MANAGE_DOCUMENTS_REQUEST_CODE = 556;
...
else if (!cordova.hasPermission(Manifest.permission.MANAGE_DOCUMENTS)) {
cordova.requestPermissions(this, MANAGE_DOCUMENTS_REQUEST_CODE,
...
@Override
public void run() {
callbackContext.error("Please allow plugin to make block working");
}
});
return;
}
...
checkGalleryPermission();
break;
case MANAGE_DOCUMENTS_REQUEST_CODE:
checkGalleryPermission();
break;
}
}
private void executeVideoDialogEvent() throws JSONException {
if (args.length() == 0) {
...
final String uriString = args.getString(0);
if (args.length() == 1) {
logger.logEvent(uriString);
callbackContext.success();
return;
}
...
ShareVideo shareVideo = new ShareVideo.Builder().setLocalUrl(selectedImageUri).build();
...
{
callbackContext.error("Not able to open facebook dialog.");
}
